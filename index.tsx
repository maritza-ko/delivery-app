...
// Per the guidelines, the API key is sourced from environment variables
// and should not be managed in the UI.
const ai = new GoogleGenAI({ apiKey: process.env.API_KEY! });

// --- MOCK DATA & HELPERS ---

const MOCK_AOV = 21500; // Mock Average Order Value
...
const App = () => {
    const [state, setState] = React.useState({
        ...
        goalType: 'sales', // 'sales', 'orders', 'aov'
        goalInput: '1,500,000',
    ...
    React.useEffect(() => {
        ...
        const interval = setInterval(() => {
            setState(prevState => {
                ...
                const newSales = newOrderCount * MOCK_AOV;
    ...
    const generateAiRecommendation = async (userGoal: number, goalType: string, location: string): Promise<string> => {
...
4.  **Two-Track Goal Setting:** You must perform two independent analyses:
...
\`\`\`json
{
  ...
  "balanced": {
    ...
    "platforms": [
      {
        "platform_name": "<'배달의민족' | '쿠팡이츠' | '요기요'>",
        "strategies": [{"type": "<string>", "value": "<string>", "reasoning": "<string>"}],
        "policy_tip": {"title": "<string>", "description": "<string>"},
        "platform_expected_sales": <number>,
        "platform_expected_orders": <number>,
        "platform_expected_new_orders": <number>,
        "platform_expected_returning_orders": <number>,
        "cost_breakdown": [
          {
            "item_category": "<'수수료'|'광고비'|'프로모션'|'배달료'>",
            "item_name": "<string: e.g., '오픈리스트 (6.8%)' or '할인 쿠폰'>",
            "item_cost": <number>
          }
        ]
      }
  ...
  "cost_saving": { /* ... same structure ... */ }
}
\`\`\``;

        const userPrompt = `우리 가게 위치는 '${location}'이고, 오늘의 목표는 ${goalType} 기준 ${formatCurrency(userGoal)}원 입니다. 이 정보를 바탕으로, 해당 지역 상권을 분석하여 균형, 공격적, 비용 절감 시나리오를 포함한 최적의 배달 전략을 한국어로 생성해주세요. 식자재 비용은 매출의 40%로 고정되므로 추정할 필요 없습니다.`;
        ...
        try {
            ...
            // FIX: The `text` field on the response is a property, not a method.
    ...
    const parseAndValidateRecommendation = (jsonString: string): Recommendation => {
        ...
        // Basic validation
        if (!data.balanced?.goal || !data.aggressive?.platforms || !data.cost_saving?.cost_assumptions || !data.realistic_analysis?.realistic_goal) {
    ...
    const getRecommendation = React.useCallback(async () => {
        ...
        try {
            const goalValue = state.goalType === 'sales' ? parseCurrency(state.goalInput) : Number(state.goalInput);
            if (isNaN(goalValue) || goalValue <= 0) {
                 throw new Error("유효한 목표를 설정해주세요.");
            }
            
            let jsonString = await generateAiRecommendation(goalValue, state.goalType, state.locationName);
            ...
            try {
                ...
                setState(prevState => ({ ...prevState, recommendation: recommendationData, loading: false }));
                 ...
                 console.warn("Self-correction attempt: Initial parsing failed. Asking AI to fix JSON.");
                 const fixPrompt = `The following JSON is invalid. Please fix it and return only the valid JSON object. Do not add any commentary or markdown. \n\nInvalid JSON:\n${jsonString}`;
                 const fixResponse = await ai.models.generateContent({
                    model: 'gemini-2.5-flash',
                    contents: [{ role: 'user', parts: [{ text: fixPrompt }] }]
                 });
                 const fixedJsonString = fixResponse.text;
                 const recommendationData = parseAndValidateRecommendation(fixedJsonString);
                 setState(prevState => ({ ...prevState, recommendation: recommendationData, loading: false }));
            }
    ...
    }, [state.goalType, state.goalInput, state.locationName]);
    ...
    const renderActiveStrategy = React.useCallback(() => {
            ...
            <div class="strategy-container">
                <div class="ai-goal-summary">
                    <div class="summary-item">
                        ...
                        <span class="value">${state.goalInput} ${state.goalType === 'orders' ? '건' : '원'}</span>
                    </div>
        ...
        `;
    }, [state.recommendation, state.activeStrategy, state.goalInput, state.goalType]);
    ...
    React.useEffect(() => {
        ...
        const adjustGoal = (amount: number) => {
            let currentValue = (state.goalType === 'sales') ? parseCurrency(state.goalInput) : Number(state.goalInput);
            let newValue = currentValue + amount;
            if (newValue < 0) newValue = 0;
            const formattedNewValue = (state.goalType === 'sales') ? formatCurrency(newValue) : String(newValue);
            setState(prevState => ({ ...prevState, goalInput: formattedNewValue }));
        };
        
        const handleIncreaseGoal = () => adjustGoal(state.goalType === 'sales' ? 100000 : 5);
        const handleDecreaseGoal = () => adjustGoal(state.goalType === 'sales' ? -100000 : -5);
        const handleFocus = () => salesGoalInput.select();

        const handleGoalTypeClick = (e: Event) => {
            const newType = (e.currentTarget as HTMLElement).dataset.type as 'sales' | 'orders' | 'aov';
            let newGoalInput = state.goalInput;
            // Sensible default conversion when switching
            if(newType === 'orders' && state.goalType === 'sales') {
                newGoalInput = String(Math.round(parseCurrency(state.goalInput) / MOCK_AOV));
            } else if (newType === 'sales' && state.goalType === 'orders') {
                newGoalInput = formatCurrency(Number(state.goalInput) * MOCK_AOV);
            }
            setState(prevState => ({ ...prevState, goalType: newType, goalInput: newGoalInput }));
        };
        ...
        const goalTypeButtonHandler = (e: Event) => handleGoalTypeClick(e);
        goalTypeButtons.forEach(btn => {
            btn.addEventListener('click', goalTypeButtonHandler);
        });
        ...
        return () => {
            ...
            goalTypeButtons.forEach(btn => {
                btn.removeEventListener('click', goalTypeButtonHandler);
            });
        };

    }, [state.goalType, state.goalInput, state.locationName, getRecommendation]); 
    ...
    }, [state.recommendation, state.activeStrategy, state.error, renderActiveStrategy, renderDailyProjection, renderStrategySummary]);
    ...
    React.useEffect(() => {
        ...
        if (state.goalType === 'sales') {
            goalLabel.textContent = '오늘의 희망 목표 매출액 (원)';
            salesGoalInput.placeholder = '예: 1,500,000';
        } else if (state.goalType === 'orders') {
            goalLabel.textContent = '오늘의 희망 목표 주문 건수 (건)';
            salesGoalInput.placeholder = '예: 70';
        } else {
            goalLabel.textContent = `오늘의 희망 목표 객단가 (AOV: ${formatCurrency(MOCK_AOV)}원)`;
             salesGoalInput.placeholder = '예: 23,000';
        }

        document.querySelectorAll('.goal-type-btn').forEach(btn => {
            btn.classList.toggle('active', btn.getAttribute('data-type') === state.goalType);
        });

    }, [state.goalType, state.goalInput]);
    ...
    }, [state.goalInput, state.goalType]);
